"""
Built upon nr_nocs
here we have the multi-view version
in this case we use all the views in one frame to do the reconstruction
Detail is :
    1.For each view of input, we predict the pnnocs independently,
      output the pnnocs point cloud and the features(inter, out)
    2.Take the union for all the features, and then aggregate them
      through a pointnet
    3.Finally, we feed the aggregated features into the IF-Net, get the occupancy
"""
import os
import glob
import torch
