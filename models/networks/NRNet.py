...
class NRNet(nn.Module):
    def __init__(self, config, device=torch.device("cpu"), is_unpooling=True, Args=None, pretrained=True, withSkipConnections=False,Sample=False):
        ...
        elif config.NRNET_TYPE == "inter_feature": # this is  not working as well as the out features
    ...
    def visualize(self, FeatureVoxel, loc, scale):
        feature_sample = FeatureVoxel[0]
        voxel = (feature_sample != 0).sum(dim=0).to(dtype=torch.bool)
        off_path = "/workspace/nrnocs_dev/debug/mid.off"
        gt_path = "/workspace/nrnocs_dev/debug/gt.off"
    ...
    def save_mesh(self, pointcloud):
        export_path = "/workspace/nrnocs_dev/debug/mid_pc.xyz"        
